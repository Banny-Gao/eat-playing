<template>
  <view class="page">
    ...
    <view class="search-wraper" v-if="tabStatus === 0">
      <image
      ...
      />
      <text v-if="showPlaceholder" class="placeholder-text">请输入城市名称</text>
      <input
        type="text"
        class="search-input"
        confirm-type="搜索"
    ...
    <scroll-view id="scrollView" scroll-y :scroll-into-view="viewId" @scroll="handleScroll">
      <view
        id="top"
        class="city-wraper"
      ...
      >
        ...
        <view class="panel-item-wraper">
          <view
            ...
            @click="setLocation(child)"
            >{{ child }} 
          </view>
        </view>
      </view>
      <template v-for="item of cityList" >
        <view class="title" :id="item.initial" :key="item.initial">{{ item.initial }}</view>
        <view
          :id="city.id"
          class="city-item"
    ...
    <view class="side-bar" v-if="tabStatus === 0">
      <view
        class="side-bar-item"
        v-for="item of sideBarList"
        :key="item.id"
        :class="getSideBarActive(item) ? 'active' : ''"
        @click="groupNavigate(item)"
      >
        {{ item.title }}
      </view>
    </view>
...
const throttleScrollFunc = throttle(function (e){
...
const citySearch = debounce(function (q, list) {
  ...
  const matchCity = (keyword, list) => {
    let i = 0, bestMatch
    let matchIndex = -1, bestMatchIndex = -1
...
export default {
  ...
  computed: {
    ...
    selectorIds() {
      return this.cityList.reduce((result, item) => {
  ...
  methods: {
    ...
    handleScroll(e) {
      throttleScrollFunc.call(this, e)
    },
  ...
  async mounted() {
    ...
    timeTask.run(() => {
      uni.createSelectorQuery().in(this).select('#scrollView').boundingClientRect(({ height }) => {
